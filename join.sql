...
SELECT DISTINCT `departments`.`name` AS `Nome Dipartimento`, `teachers`.`surname` AS `Cognome docente`, `teachers`.`name` AS `Nome docente` 
FROM `departments` 
JOIN `degrees` 
ON `departments`.`id` = `degrees`.`department_id` 
JOIN `courses` 
...
LIKE 'Dipartimento di Matematica' 
